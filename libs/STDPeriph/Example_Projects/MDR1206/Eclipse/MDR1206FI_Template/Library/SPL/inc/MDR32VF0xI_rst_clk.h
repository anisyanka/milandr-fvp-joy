...
typedef struct
{
    ...
    uint32_t UART_Frequency[4];      /*!< UART frequencies after a prescaler. */
    uint32_t TIMER_Frequency[4];     /*!< TIMER frequencies after a prescaler. */
    uint32_t SSP_Frequency[3];       /*!< SSP frequencies after a prescaler. */
    uint32_t WWDG_Frequency;         /*!< WWDG frequency after a prescaler. */
    uint32_t ADC_Frequency;          /*!< ADC frequency after a prescaler. */
    uint32_t ADCUI_Frequency;        /*!< ADCUI frequency after a prescaler. */
    uint32_t RTCHSI_Frequency;       /*!< HSI_RTC frequency after a prescaler. */
    uint32_t RTCHSE_Frequency;       /*!< HSE_RTC frequency after a prescaler. */
 ...
 */
typedef enum {
    RST_CLK_PLLCPU_CLK_SRC_HSE              = ((uint32_t)RST_CLK_PLL_CONTROL_PLL_CPU_SEL),
    RST_CLK_PLLCPU_CLK_SRC_CPU_C1_HSI       = ((uint32_t)RST_CLK_CPU_CLOCK_CPU_C1_SEL_HSI),
    RST_CLK_PLLCPU_CLK_SRC_CPU_C1_HSI_DIV_2 = ((uint32_t)RST_CLK_CPU_CLOCK_CPU_C1_SEL_HSI_DIV_2),
    RST_CLK_PLLCPU_CLK_SRC_CPU_C1_HSE       = ((uint32_t)RST_CLK_CPU_CLOCK_CPU_C1_SEL_HSE),
    RST_CLK_PLLCPU_CLK_SRC_CPU_C1_HSE_DIV_2 = ((uint32_t)RST_CLK_CPU_CLOCK_CPU_C1_SEL_HSE_DIV_2)
} RST_CLK_PLLCPU_ClockSource_TypeDef;

#define IS_RST_CLK_PLLCPU_CLOCK_SOURCE(CLK) (((CLK) == RST_CLK_PLLCPU_CLK_SRC_HSE) ||              \
                                             ((CLK) == RST_CLK_PLLCPU_CLK_SRC_CPU_C1_HSI) ||       \
                                             ((CLK) == RST_CLK_PLLCPU_CLK_SRC_CPU_C1_HSI_DIV_2) || \
                                             ((CLK) == RST_CLK_PLLCPU_CLK_SRC_CPU_C1_HSE) ||       \
                                             ((CLK) == RST_CLK_PLLCPU_CLK_SRC_CPU_C1_HSE_DIV_2))
...
/**
 * @brief RST_CLK PLL_CPU_MUL multiplier constants of the CPU_C1 clock.
 */
typedef enum {
...
typedef enum {
    ...
    RST_CLK_PCLK_IWDG    = RST_CLK_PER2_CLOCK_PCLK_EN_IWDT,
...
#define IS_RST_CLK_PCLK(PCLK) (((PCLK) == RST_CLK_PCLK_SSP1) ||        \
                               ...
                               ((PCLK) == RST_CLK_PCLK_SENSORS) ||     \
                               ((PCLK) == RST_CLK_PCLK_CLK_MEASURE) || \
...
#define RST_CLK_PCLK_MASK (RST_CLK_PCLK_SSP1 | RST_CLK_PCLK_UART1 |          \
                           ...
                           RST_CLK_PCLK_SENSORS | RST_CLK_PCLK_CLK_MEASURE | \
...
typedef enum {
    ...
    RST_CLK_DMA_DONE_STICK_CHANNEL_SW5      = RST_CLK_DMA_DONE_STICK_DMA_DONE_STICK31  /*!< Software channel 5. */
