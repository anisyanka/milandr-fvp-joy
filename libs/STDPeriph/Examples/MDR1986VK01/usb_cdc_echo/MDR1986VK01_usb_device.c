...
#define ASSERT_INFO_FILE_ID    FILEID__MDR1986VK01_USB_DEVICE_C
...
USB_Result USB_EP_doDataIn( USB_EP_TypeDef EPx, uint8_t* Buffer, uint32_t Length, USB_EP_IO_Handler onInDone )
{
    ...
    if ( EPx == USB_EP0 )
    {
        if ( Length > MAX_PACKET_SIZE )
        {
            ...
            StartInStage = USB_DATA_BIT_KEEP;
        }
        else
        {
            ep->EP_State = USB_EP_IDLE;
            StartInStage = USB_DATA_BIT_TOGGLE;
        }
    }
    else
    {
...
//    uint32_t tmpSTS, tmpTS, tmpCTRL, i;
    uint32_t               count;
...
/* END OF FILE MDR1986VK01_usb_device.c */
