...
/** @addtogroup BKP_RTC_234 BKP RTC example for K1986VK234 evaluation board
 *  @{
 */
...
volatile uint32_t AlarmValue = ALARM_VALUE - 1;
...
void BKP_RTC_IRQHandler(void)
{
    ...
    if (BKP_RTC_GetITFlagStatus(BKP_RTC_IT_ALRF) != RESET) {
        ...
        PORT_Write(MDR_PORTC, (PORT_ReadInputData(MDR_PORTC) & ~(0x7 << 5)) | ((Cnt & 0x7) << 5));
        BKP_RTC_SetAlarm(AlarmValue);
...
int main(void)
{
    ...
    PORT_InitStructure.PORT_Pin       = (PORT_PIN_4 | PORT_PIN_5 | PORT_PIN_6 | PORT_PIN_7);
