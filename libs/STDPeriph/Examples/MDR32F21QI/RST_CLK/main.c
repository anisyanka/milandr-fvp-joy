...
int main(void)
{
    ...
    /* Configure PORTC pins 4..7 for output to switch LEDs on/off */
    ...
    DELAY_PROGRAM_WaitLoopsAsm(4 * LED_PERIOD);
    PORT_SetBits(MDR_PORTC, LED3);
    DELAY_PROGRAM_WaitLoopsAsm(4 * LED_PERIOD);
    PORT_ResetBits(MDR_PORTC, LED3);
    DELAY_PROGRAM_WaitLoopsAsm(4 * LED_PERIOD);
    PORT_SetBits(MDR_PORTC, LED4);
    DELAY_PROGRAM_WaitLoopsAsm(4 * LED_PERIOD);
    PORT_ResetBits(MDR_PORTC, LED4);
    DELAY_PROGRAM_WaitLoopsAsm(4 * LED_PERIOD);
    ...
    if (BKP_HSI_GetStatus() == SUCCESS) /* Good HSI clock */
    {
        /* Switch LED1 off */
        PORT_ResetBits(MDR_PORTC, LED1);
        ...
        /* Setup internal DUcc voltage regulator work mode based on clock frequency */
        BKP_DUcc_SetMode(BKP_DUCC_CLK_UPTO_10MHz);
        /* Setup EEPROM access delay to 0: HSI = 8MHz < 18MHz */
        EEPROM_SetLatency(EEPROM_LATENCY_CYCLE_0);

        /* Select HSI clock on the CPU clock MUX */
        RST_CLK_HCLK_ClkSelection(RST_CLK_CPU_CLK_HSI);
    ...
    if (RST_CLK_HSE_GetStatus() == SUCCESS) /* Good HSE clock */
    {
        ...
        if (RST_CLK_CPU_PLL_GetStatus() == SUCCESS) /* Good CPU PLL */
        {
            ...
            /* LED2 blinking with 4*HSE/2 clock as input clock source */
            BlinkLED(BLINK_NUM, LED_PERIOD, LED2);
    ...
    BKP_LSI_Cmd(ENABLE);
    /* Switch LED3 on and wait for LSI ready status */
    PORT_SetBits(MDR_PORTC, LED3);
    DELAY_PROGRAM_WaitLoopsAsm(LED_PERIOD);
    if (BKP_LSI_GetStatus() == SUCCESS) /* Good LSI clock */
    {
        ...
        /* LED3 blinking with LSI clock as input clock source */
        BlinkLED(BLINK_NUM, LED_PERIOD / 100, LED3);
    } else /* LSI timeout */
    {
        while (1) { }
    }
    ...
    /* Switch LED1 on and wait for LSE ready status */
    PORT_SetBits(MDR_PORTC, LED4);
    DELAY_PROGRAM_WaitLoopsAsm(LED_PERIOD / 100);
    if (BKP_LSE_GetStatus() == SUCCESS) /* Good LSE clock */
    {
        ...
        /* LED4 blinking with LSI clock as input clock source */
        BlinkLED(BLINK_NUM, LED_PERIOD / 100, LED4);
    } else /* LSE timeout */
    {
        while (1) { }
    }
