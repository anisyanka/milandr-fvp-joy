...
/* Private variables ---------------------------------------------------------*/
static uint16_t CH1_CCR_Val = (uint16_t)((double)(TIMER_CNT_PERIOD + 1) * 25.0) / 100;
...
int main(void)
{
    ...
    /* Configure PORTA[1:0] (TMR0_CH1/CH1N) for TIMER0 */
    ...
    /* Configure TIMER0 ---------------------------------------------------
    Generates 2 PWM signals (CH1 and CH1N)
    ...
    - PWM duty cycle = CCR1 / (CounterPeriod + 1): CH1 = 25% and CH1N = 75%
    -----------------------------------------------------------------------*/
    RST_CLK_PCLKCmd(RST_CLK_PCLK_TIMER0, ENABLE);

    RST_CLK_HSI_C1_SetPrescaler(RST_CLK_HSI_PRESCALER_DIV_1);
    RST_CLK_PER1_C2_ClkSelection(RST_CLK_PER1_C2_TIM0, RST_CLK_PER1_C2_CLK_SRC_HSI_CLK);
